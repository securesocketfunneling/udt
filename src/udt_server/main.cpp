...
int main(int argc, char* argv[]) {
  if (argc != 2) {
    BOOST_LOG_TRIVIAL(error) << "Command help : ./udt_client [port]";
    return 1;
  }
  ...
  accepted = [&](const boost::system::error_code& ec, p_socket_type p_socket) {
    if (ec) {
      BOOST_LOG_TRIVIAL(trace) << "Not accepted : ec : " << ec.value() << " "
                               << ec.message();
      return;
    }

    BOOST_LOG_TRIVIAL(trace) << "Accepted" << std::endl;
    boost::asio::async_read(*p_socket, boost::asio::buffer(r_buffer2),
                            boost::bind(received_handler, _1, _2, p_socket));

    p_socket_type p_new_socket(std::make_shared<udt_protocol::socket>(io_service));
  ...
  received_handler = [&](const boost::system::error_code& ec,
    ...
    if (ec) {
      BOOST_LOG_TRIVIAL(trace) << "receive ec : " << ec.value() << " "
                               << ec.message();
      return;
    }
